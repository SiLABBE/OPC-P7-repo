...
def global_FI_plot(n_top=20):
    ...
    plt.box(False) 
    lr_title = plt.suptitle("Global importance of main parameters (Top " + str(top_x) + ")", fontsize=20, fontweight="normal")
    st.pyplot()
...
def main():
    ...
    feature_select = st.sidebar.multiselect("Select 2 features to see customer position", features_list, default=["AMT_GOODS_PRICE", "EXT_SOURCE_2"])
    ...
    if customer_btn:
        if int(selected_customer) in customer_list:
            ...
            multi_features_plot(df_customer, feature_select[0], feature_select[1], filtered_customer)

            shap_obj = shap.Explanation(np.array(shap_values), base_values=np.array(shap_base_value), feature_names=features_list)
